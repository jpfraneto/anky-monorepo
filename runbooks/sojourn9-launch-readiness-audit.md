...
| Executable privacy guard for launch metadata | `solana/scripts/sojourn9/privacyGuard.mjs`, `privacyGuard.test.mjs`, `launchReadinessGate.mjs` | Locally covered; guard checks migrations `019-021`, backend verified/webhook guards, operator/indexer script logging/options, stderr secret redaction, and mobile proof-state plus backup manifest code for private `.anky`/witness persistence regressions; it does not inspect secret files or private `.anky` contents |
...
1. Apply `migrations/019_mobile_verified_seal_receipts.sql`, `migrations/020_mobile_helius_webhook_events.sql`, and `migrations/021_mobile_helius_webhook_signature_dedupe.sql` to the target backend database.
