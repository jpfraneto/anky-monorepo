...
| Executable privacy guard for launch metadata | `solana/scripts/sojourn9/privacyGuard.mjs`, `privacyGuard.test.mjs`, `launchReadinessGate.mjs` | Locally covered; guard checks VerifiedSeal backend migrations `019_mobile_verified_seal_receipts` through `021_mobile_helius_webhook_signature_dedupe` plus `022_credit_ledger_entries`, backend verified/webhook guards, operator/indexer script logging/options, stderr secret redaction, and mobile proof-state plus backup manifest code for private `.anky`/witness persistence regressions; it does not inspect secret files or private `.anky` contents |
...
| Score V1 allocation | `score = unique_seal_days + (2 * verified_seal_days) + streak_bonus` in indexer/runbook | Locally covered; allocation defaults to an auditable 3,456 participant cap with deterministic score-desc/wallet-asc tie handling; token supply/snapshot time still launch inputs |
...
1. Apply `migrations/019_mobile_verified_seal_receipts.sql`, `migrations/020_mobile_helius_webhook_events.sql`, `migrations/021_mobile_helius_webhook_signature_dedupe.sql`, and `migrations/022_credit_ledger_entries.sql` to the target backend database. VerifiedSeal backend readiness specifically requires `019`, `020`, and `021`; `022_credit_ledger_entries` is part of the full backend migration chain.
