...
\$ANKY is designed to participate in the Anky ecosystem without being required for it. The practice is free. The token is optional. But holding \$ANKY unlocks:

\begin{enumerate}[leftmargin=2cm]
    \item \textbf{Premium access.} Holding a threshold amount of \$ANKY grants premium status --- instant Claude-powered meditation and breathwork generation, no queue.

    \item \textbf{Governance.} As the facilitator network grows, \$ANKY holders participate in governance decisions: which facilitators to spotlight, how to allocate community funds, what features to prioritize.

    \item \textbf{Community identity.} Every anky (8-minute writing artifact) is a potential on-chain artifact. The image generated from the writing, the flow score, the timestamp --- these can be minted as NFTs on Solana, creating a permanent, public record of the practice.

    \item \textbf{Economic alignment.} Revenue from facilitator bookings (the 8\% platform fee) flows to a treasury. The treasury is governed by \$ANKY holders. This creates a direct economic link between the growth of the facilitator network and the value of the token.
\end{enumerate}
...
\subsection{Treasury and Sustainability}

The Anky treasury receives:

\begin{itemize}[leftmargin=2cm]
    \item 8\% of every facilitator booking
    \item Revenue from premium subscriptions
    \item Optional donations from users who want to support the free tier
\end{itemize}

The treasury funds:

\begin{itemize}[leftmargin=2cm]
    \item AI inference costs (Claude API for premium generation)
    \item Infrastructure (electricity, internet, hardware replacement)
    \item Facilitator vetting and onboarding
    \item Development of new features
    \item Grants for facilitators serving underserved communities
\end{itemize}

The treasury address is on Base (Ethereum L2) and receives USDC. It is publicly auditable. The goal is for the facilitator network's revenue to eventually make the entire system self-sustaining --- the premium users and facilitator bookings funding the free tier indefinitely.
