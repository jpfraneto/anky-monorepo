...
pub async fn dataset_round_two(
    State(state): State<AppState>,
) -> Result<Html<String>, AppError> {
    ...
    if let Ok(entries) = fs::read_dir(gi_dir) {
        let mut names: Vec<String> = entries
            ...
            .filter_map(|e| {
                ...
                if n.ends_with(".png") { Some(n[..n.len()-4].to_string()) } else { None }
            })
            .collect();
        names.sort();
        for id in names {
            ...
            if src.exists() && !thumb.exists() { make_thumb_200(&src, &thumb); }
    ...
    if let Ok(batches) = fs::read_dir(gens_dir) {
        ...
        for batch_dir in batch_dirs {
            ...
            let batch_id = batch_dir.file_name()
            ...
            if let Ok(content) = fs::read_to_string(&review_path) {
                if let Ok(map) = serde_json::from_str::<serde_json::Value>(&content) {
                    if let Some(obj) = map.as_object() {
                        let mut approved: Vec<&String> = obj.iter()
                            .filter(|(_, v)| v.get("decision").and_then(|d| d.as_str()) == Some("approved"))
                            .map(|(k, _)| k)
                            .collect();
                        approved.sort();
                        for image_id in approved {
                            let src = PathBuf::from(format!("data/generations/{}/images/{}.png", batch_id, image_id));
                            let thumb_dir = PathBuf::from(format!("data/generations/{}/thumbs", batch_id));
...
pub async fn dataset_eliminate(
    ...
    struct Sel { source: String, batch_id: String, image_id: String }
    ...
    for sel in &selections {
        if sel.batch_id.contains("..") || sel.image_id.contains("..") { continue; }
        ...
        if sel.source == "gallery-images" {
            ...
            if thumb_src.exists() { let _ = fs::rename(&thumb_src, &thumb_dst); }
            moved += 1;
            continue;
        }
        ...
        let thumb_src = PathBuf::from(format!("data/generations/{}/thumbs/{}.webp", sel.batch_id, sel.image_id));
        let thumb_dst = dst_dir.join(format!("{}.webp", sel.image_id));
        if thumb_src.exists() { let _ = fs::rename(&thumb_src, &thumb_dst); }
        ...
        if let Ok(content) = fs::read_to_string(&review_path) {
            if let Ok(mut map) = serde_json::from_str::<serde_json::Value>(&content) {
                if let Some(entry) = map.get_mut(&sel.image_id) {
                    ...
                    let _ = fs::write(&review_path, serde_json::to_string(&map).unwrap_or_default());
                }
            }
        }
